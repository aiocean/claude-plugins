...
description: "Bridge Claude to a browser UI in real time. The technical problem: Claude runs in a turn-by-turn CLI loop with no event loop — it cannot addEventListener on a tab, cannot block on user input. This skill ships a frozen Bun + Vue3 + Tailwind scaffold that solves it via two channels: browser events become Monitor-tool notifications (server emits MSG:: lines to stdout, Monitor pattern-matches → Claude wakes), AI pushes become WebSocket broadcasts (POST /api/push → all browser tabs). Claude only writes the APP REGION of app.html; the runtime, message protocol, and vendor blocks are frozen. Use when an interactive browser UI must be driven by Claude while a task is in progress — form capture, multi-step decision flow, live preview, approval queue, side-by-side review. Triggers: \"/aio-html-interactive\", \"/interactive\", \"interactive UI for AI\", \"browser ↔ AI realtime\", \"AI-driven UI\", \"Monitor + WebSocket bridge\", \"làm cái UI tương tác với AI\", \"dựng UI tương tác\", \"realtime browser AI\"."
document_type: "skill"
plugin: "aio-html-interactive"
install: "/plugin install aio-html-interactive@aiocean-plugins"
---

> From plugin [**aio-html-interactive**](/vi/plugins/aio-html-interactive) · `v1.0.1` · **Install:** `/plugin install aio-html-interactive@aiocean-plugins`
