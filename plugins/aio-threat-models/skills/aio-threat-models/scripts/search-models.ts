#!/usr/bin/env npx tsx
/**
 * Semantic search across threat modeling frameworks using pre-computed embeddings.
 * Uses snowflake-arctic-embed-xs for query embedding, cosine similarity against embeddings.json.
 *
 * Usage: npx tsx search-models.ts "privacy threats in ML system"
 * Options:
...
async function main() {
  ...
  if (!query) {
    console.error("Usage: npx tsx search-models.ts \"your query here\" [--top N] [--json]");
    process.exit(1);
  }
