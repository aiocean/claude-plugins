...
Note: Always make sure to take into account [The Standard of Code Review](reviewer-01-standard.md) when considering each of these points.
...
Ask for unit, integration, or end-to-end tests as appropriate for the change. In general, tests should be added in the same CL as the production code unless the CL is handling an [emergency](02-emergencies.md).
...
What if the existing code is inconsistent with the style guide? Per our [code review principles](reviewer-01-standard.md), the style guide is the absolute authority: if something is required by the style guide, the CL should follow the guidelines.
...
In these cases, note in a comment which parts you reviewed. Prefer giving [LGTM with comments](reviewer-04-speed.md#lgtm-with-comments).
