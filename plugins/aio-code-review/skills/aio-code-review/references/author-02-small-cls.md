...
- The CL should include related test code (see [Keep related test code in the same CL](#keep-related-test-code-in-the-same-cl)).
...
CLs should include related test code. Remember that "smallness" here refers to the conceptual idea that the CL should be focused and is not a simplistic function on line count.
...
*Independent* test modifications can go into separate CLs first, similar to the [refactorings guidelines](#separate-out-refactorings). That includes:
