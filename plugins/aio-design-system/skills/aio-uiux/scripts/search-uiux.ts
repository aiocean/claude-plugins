#!/usr/bin/env npx tsx
/**
 * Semantic search across UI/UX knowledge base using pre-computed embeddings.
 * Uses snowflake-arctic-embed-xs for query embedding, cosine similarity against embeddings.json.
 *
 * Usage: npx tsx search-uiux.ts "how to design accessible forms"
 * Options:
...
async function main() {
  ...
  if (!query) {
    console.error('Usage: npx tsx search-uiux.ts "your query here" [--top N] [--json]');
    process.exit(1);
  }

  // Load pre-computed embeddings
  const raw = await readFile(EMBEDDINGS_FILE, "utf-8");
  const topics: TopicEmbedding[] = JSON.parse(raw);
  ...
  const scored = topics.map((t) => ({
    ...
    category: t.category,
    file: t.file,
    score: cosineSimilarity(queryVector, t.vector),
  }));
  ...
  if (json) {
    ...
    for (const r of results) {
      const catLabel = r.category.replace(/-/g, " ");
      console.log(`  ${r.rank}. ${r.title} (${catLabel}) — score: ${r.score}`);
    }
