...
| **aio-message-bridge** | 1.0.0 | Give Claude Code an event loop — the generic transport beneath `aio-html-interactive`. A local HTTP + WebSocket relay + the Monitor tool let anything outside Claude (CLI, mobile app, webhook, another program, web page) talk to it mid-task both ways. Inbound: client POSTs an event → relay prints `MSG::` to stdout → Monitor notification. Outbound: Claude POSTs a push → relay broadcasts over WebSocket. Language-agnostic, with a ready-to-run reference relay + vanilla client; localhost by default, optional token-gated exposure over Cloudflare Tunnel / Tailscale / LAN.<br>`npx skills add aiocean/claude-plugins -s aio-message-bridge` |
